# The purpose of this script is to compare the results of ligand-receptor 
# interaction analysis between two conditions (e.g., CTRL vs CF) using 
# the CellChat objects created in the previous step. 
# This script will generate various visualizations to identify altered 
# interactions and cell populations, as well as signaling networks with
# distinct architecture and interaction strength.

# this is the "cli" version of the script, which is meant to be run on 
# the command line
...
use_condaenv(
  "cellchat-env",
  #conda = "/mnt/data/project0062/miniforge3/bin/conda",
  conda = "/opt/gridware/depots/996bcebb/el8/pkg/apps/miniforge/24.7.1/bin/bin/conda",
  required = TRUE
)
...
parser <- ArgumentParser(
  description = "Compare the results of ligand-receptor interaction analysis between two conditions using CellChat objects."
)
...
parser$add_argument(
  "--input_dir", 
  type = "character", 
  default = "results/comb/lr-manual-niches/5", 
  help = "Directory containing the input CellChat object RDS files"
)

parser$add_argument(
  "--output_dir", 
  type = "character", 
  default = "results/comb/lr-manual-niches/5/CTRL_vs_CF", 
  help = "Directory to save the output comparison results and visualizations"
)
...
parser$add_argument(
  ...
  type = "character", 
  default = "CTRL", 
  help = "Name of the first group/condition (e.g., CTRL)"
)
...
parser$add_argument(
  ...
  type = "character", 
  default = "CF", 
  help = "Name of the second group/condition (e.g., CF)"
)
...
# source the CellChat functions script, with error handling
...
obj_paths <- setNames(
  c(
    file.path(IN_DIR, paste0(REF, ".cellchat.rds")),
    file.path(IN_DIR, paste0(CONT, ".cellchat.rds"))
  ),
  c(REF, CONT)
)
...
horizontal_plot_settings <- list(
  width = 25,
  height = 10
)

single_plot_settings <- list(
  width = 10,
  height = 10
)
...
# we do this to ensure that the cell types are consistent across the 
# two CellChat objects, which is a requirement for merging and comparing 
# them. If the cell types are not consistent, we will get an error when 
# trying to merge or compare the objects.
...
# sanity check to ensure that the cell types are now consistent 
# across the two CellChat objects
...
# calculate centrality scores before merging
# if not already done
...
png(
  ...
  width = horizontal_plot_settings$width, 
  height = horizontal_plot_settings$height, 
  res = res,
  units = units
)
# plot 
...
png(
  ...
  width = single_plot_settings$width, 
  height = single_plot_settings$height, 
  res = res,
  units = units
)
# plot
...
# TODO: Change this later to take the mapping from the metadata of the cellchat object
# DID: something similar, takes a rds object that is modifiable
celltype_config <- readRDS('code/8_lr_analysis/celltype_config.rds')

map.celltype_group <- celltype_config$map.celltype_group
levels.group <- celltype_config$levels.group

# coarse group per fine cell type
...
# levels.group <- c(
#   "EPI", "MYELOID", "LYMPHOID",
#   "ENDOTHELIAL", "STROMAL", "OTHER"
# )

cellchat_list_grouped <- lapply(cellchat_list, function(cellchat_obj) {

  fine_celltypes <- levels(cellchat_obj@idents)

  group.cellType <- map.celltype_group[fine_celltypes]
  ...
  mergeInteractions(cellchat_obj, group.cellType)
})
...
celltypes <- levels(cellchat_merged@meta$ann_lvl_3_refined)
...
for (pathway in names(cellchat_list[[i]]@netP$centr)) {
  ...
  # incoming centrality
  cellchat_list[[i]]@netP$centr[[pathway]]$indeg <-
    cellchat_list[[i]]@netP$centr[[pathway]]$indeg + epsilon
}
...
print("Finished plotting signaling patterns heatmaps.")
...
tryCatch({
  ...
  # code for diff. exp. analysis goes here
